// Inline SVG icons (no external requests). Usage: icon('github', 'my-class')
...
const FILLED = {
  ...
  codeforces: {
    viewBox: '0 0 24 24',
    body: '<rect x="2" y="9" width="5.5" height="12" rx="1.2"/><rect x="9.25" y="3" width="5.5" height="18" rx="1.2"/><rect x="16.5" y="12" width="5.5" height="9" rx="1.2"/>',
  },
};
...
export function icon(name, className = '') {
  ...
  if (filled) {
    return `<svg${cls} viewBox="${filled.viewBox}" fill="currentColor" aria-hidden="true" focusable="false">${filled.body}</svg>`;
  }
  const body = STROKE[name] ?? STROKE.code;
  return `<svg${cls} viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false">${body}</svg>`;
}

/** Fill every <span data-icon="name"> in the static HTML with its SVG. */
